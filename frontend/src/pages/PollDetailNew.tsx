...
// Format average with 1 decimal, show dash when no votes
...
export default function PollDetailNewPage() {
  ...
  const [confirmOpen, setConfirmOpen] = useState(false); // confirm poll closure
  const [distOpen, setDistOpen] = useState(false); // distribution bars
  ...
  const closePoll = useMutation({
    mutationFn: async () => apiFetch(`/polls/${id}/close`, { method: "POST" }),
    onSuccess: () => {
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6">
        ...
        {data && (
          <section className="mt-6 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
            ...
            <div className="mb-6 flex flex-wrap items-center gap-2 text-neutral-900">
              {/* Average value */}
              ...
              <button
                ...
                className="ml-2 inline-flex items-center gap-1 rounded-full border border-neutral-200 px-2.5 py-1 text-xs font-medium text-neutral-700 hover:bg-neutral-50"
              >
                {/* Mini horizontal bars icon */}
                <svg className="h-3.5 w-3.5 text-neutral-600" viewBox="0 0 20 20" aria-hidden="true">
                  <rect x="3" y="4" width="12" height="2" rx="1" fill="currentColor" />
                  <rect x="3" y="9" width="8" height="2" rx="1" fill="currentColor" />
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-80 rounded-xl bg-white p-6 shadow-xl">
        <h2 className="mb-3 text-lg font-semibold">Conferma chiusura</h2>
        <p className="mb-5 text-sm text-neutral-700">
          Chiudere definitivamente questo sondaggio?
          <br />
          <br />
...
/* Local component: simple modal to show distribution */
// Defined outside PollDetailPage to avoid re-creation on each render.
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-neutral-500">Distribuzione voti</p>
            <p className="text-sm text-neutral-700">{total} voti totali</p>
          </div>
          <button
            onClick={onClose}
            className="rounded-full border border-neutral-200 px-3 py-1 text-xs font-medium text-neutral-700 hover:bg-neutral-50"
          >
            Chiudi
          </button>
        </div>
                ...
                <div className="relative h-3 w-full overflow-hidden rounded-full bg-neutral-100">
                  <div
                    className={`h-full rounded-full ${
                      b.rating >= 4 ? "bg-yellow-400/90" : "bg-blue-400/90"
                    }`}
                    style={{ width: `${pct}%` }}
